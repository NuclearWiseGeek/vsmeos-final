...
// Define the shape of the data we expect (same as your Context)
interface ESGData {
  companyName: string;
  country: string;
  revenue: string;
...
export const generateCarbonPack = (data: ESGData) => {
  ...
  // --- CALCULATIONS (Same as Results Page) ---
  const FACTORS = {
    gas: 0.244, diesel: 3.16, petrol: 2.8,
    elec: 0.052, heat: 0.170,
    vehicle: 0.218, flight: 0.14, hotel: 6.9
  };

  // Calculate row by row for the Detailed Table
  const rows = [];
  ...
  // Totals
  const s1 = (data.gas * FACTORS.gas) + (data.diesel * FACTORS.diesel) + (data.petrol * FACTORS.petrol);
  ...
  // List required evidence dynamically based on inputs
  finalY += 5;
  if(data.gas > 0) { doc.text("- Natural Gas Invoices", 20, finalY); finalY += 4; }
  if(data.elec > 0) { doc.text("- Electricity Utility Invoices", 20, finalY); finalY += 4; }
  ...
  finalY += 15;
  doc.line(14, finalY, 80, finalY); // Signature line
